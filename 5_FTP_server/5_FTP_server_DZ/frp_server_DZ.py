...
END_FLAG = b"$$STREAM_FILE_END_FLAG$$" #флаг, обозначающий конец передачи файла
FAIL_FLAG = b'$FAILED$' #флаг, обозначающий неудачу при выполнении операции
# какой порт будет прослушиватьсяч
PORT = 6666 
global_root = os.path.join(os.getcwd()) #представляет корневую директорию сервера
usersfile = os.path.join(global_root, "users.csv") #файлы пользователей
log_file = os.path.join(global_root, "log.txt") #журнал событий

def get_size(start_path): # вычисляет размер файла или директории
    total_size = 0
    for dirpath, dirnames, filenames in os.walk(start_path):
        for f in filenames:
            fp = os.path.join(dirpath, f)
            if not os.path.islink(fp): #пропустить если это символическая ошибка
                total_size += os.path.getsize(fp)

    return total_size

#логирование действий сервера в файл(Задание №2)
#используется для вывода сообщений в консоль и записи их в журнал.
...
#обрабатывает запросы от клиента в соответствии с определенными командами (например, перемещение по директориям, создание и удаление файлов и директорий).
def process(req):
...
# функция прочитывания пути(декодирует пути файлов и директорий)
...
# принимает подключения от клиентов, получает от них запросы, обрабатывает их и отправляет ответы.
while True:
